...
export default function Invoices({ className, user, pixKey, pixName }: Props) {
  ...
  const [currentInvoice, setCurrentInvoice] = useState({
    ...
    amount: user.ipvaDebts.reduce((p, c) => p + c.value, 0),
  ...
  const total =
    user.ipvaDebts.reduce((p, c) => p + c.value, 0) +
    user.finesDebts.reduce((p, c) => p + c.value, 0) +
    (user.licensing?.value ?? 0);
  ...
  return (
    <div className={twMerge('w-full flex flex-col', className)}>
      {user.licensing ? (
        <div className='w-full py-5 px-3 bg-eaebec rounded-[4px] flex items-center justify-between gap-4 mb-4 max-[800px]:flex-col max-[500px]:gap-3'>
          <div className='flex items-center gap-2'>
            ...
            <p className='text-00823c uppercase font-bold text-lg'>
              {user.licensing.label}
            </p>
          </div>
          <div className='flex items-center gap-6 max-[500px]:flex-col max-[500px]:gap-2'>
            ...
            <div className='bg-white rounded-[6px] px-5 py-2 font-bold text-[20px] text-2c5799'>
              {formatPrice(user.licensing.value)}
            </div>
            <button
              type='button'
              onClick={() =>
                handlePaymentQRCodeStatic(user.licensing?.value ?? 0)
              }
              className='bg-2c5799 flex items-center gap-2 rounded-[6px] px-5 py-2 font-bold text-lg text-white max-[500px]:mt-2'
            >
              Pagar
              <svg
                xmlns='http://www.w3.org/2000/svg'
      ...
      {user.ipvaDebts.length ? (
        <div className='w-full py-5 px-3 bg-eaebec rounded-[4px] flex flex-col mb-4'>
          <div className='flex items-center gap-2 mb-2'>
            ...
            <p className='text-00823c uppercase font-bold text-lg'>Ipva</p>
          </div>
          <div className='grid grid-cols-2 w-full gap-8 max-[600px]:grid-cols-1 max-[600px]:gap-4'>
            {user.ipvaDebts.map((item, i) => (
              <div
                key={i}
      ...
      {user.finesDebts.length ? (
        <div className='w-full py-5 px-3 bg-eaebec rounded-[4px] flex flex-col mb-4'>
          <div className='flex items-center gap-2 mb-2'>
            ...
            <p className='text-00823c uppercase font-bold text-lg'>Multas</p>
          </div>
          <div className='grid grid-cols-2 w-full gap-8 max-[600px]:grid-cols-1 max-[600px]:gap-4'>
            {user.finesDebts.map((item, i) => (
              <div
                key={i}
                className={`w-full flex items-center justify-center flex-col gap-2 ${i % 2 === 0 ? 'border-r border-r-2c5799 border-solid max-[600px]:border-r-transparent max-[600px]:border-r-0 max-[600px]:border-b-2c5799 max-[600px]:border-b max-[600px]:pb-4' : ''}`}
              >
